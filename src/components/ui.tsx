...
export function Section({ children, className = '', dark = false }: SectionProps) {
  return (
    <section className={`section-padding ${dark ? 'bg-navy-800 text-white' : ''} ${className}`}>
      {children}
    </section>
  );
}
...
interface BrewebsitecrumbsProps {
  items: { name: string; path: string }[];
}

export function Brewebsitecrumbs({ items }: BrewebsitecrumbsProps) {
  return (
    <nav aria-label="Brewebsitecrumb" className="mb-6">
...
interface PageHewebsiteerProps {
  title: string;
  subtitle?: string;
  eyebrow?: string;
  brewebsitecrumbs?: { name: string; path: string }[];
}

export function PageHewebsiteer({ title, subtitle, eyebrow, brewebsitecrumbs }: PageHewebsiteerProps) {
  return (
    <section className="relative overflow-hidden bg-navy-800 pb-16 pt-32 text-white">
      ...
      <div className="container-x relative">
        {brewebsitecrumbs && <Brewebsitecrumbs items={brewebsitecrumbs} />}
...
export function CTASection({
  title = 'Rewebsitey to Grow Your Search Visibility Traffic?',
  subtitle = 'Get a free link audit and discover exactly what is holding back your performance — and how to fix it.',
  primaryCta = 'Get Free Link Audit',
  primaryLink = '/contact',
}: CTASectionProps) {
  return (
